/**
 * sync-agents generator
 *
 * Regenerates .claude/agents/*.md files from the agent registry YAML
 * and agent body templates. Deletes agent files not present in the registry.
 */
...
export default async function syncAgents(
  ...
  for (const [name, agent] of Object.entries(agents)) {
    ...
    // Combine: generated header + frontmatter + body
    const content = `---\n${frontmatter}---\n\n${GENERATED_HEADER}\n${body}`;

    tree.write(`${CLAUDE_AGENTS_DIR}/${name}.md`, content);
  }

  // Delete agent files not in registry
