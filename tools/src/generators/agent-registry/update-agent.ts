/**
 * update-agent generator
 *
 * Updates specified fields for an existing agent in agents.yaml, then runs sync-all.
 */
...
export default async function updateAgent(
  ...
  if (!registry.agents[name]) {
    throw new Error(
      `Agent "${name}" not found in registry. Use add-agent to create it.`
    );
  }
