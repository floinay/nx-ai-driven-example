...
export default async function addAgent(
  ...
  const { name, model, tools, description, permissionMode = 'default' } = schema;
  ...
  if (registry.agents[name]) {
    throw new Error(`Agent "${name}" already exists. Use update-agent to modify it.`);
  }
