/**
 * validate-registry generator
 *
 * Checks that all derived files (agent markdowns, skill markdowns,
 * settings.json) match what the registry would
 * generate. Reports mismatches and exits with error if any found (for CI).
 */
...
export default async function validateRegistry(
  ...
  for (const [name, agent] of Object.entries(agents)) {
    ...
    if (!tree.exists(filePath)) {
      errors.push({
        file: filePath,
        message: 'File missing (expected from registry).',
      });
      continue;
    }
    ...
    if (normalizeContent(actual) !== normalizeContent(expected)) {
      errors.push({
        file: filePath,
        message:
          'Content does not match what registry would generate. Run: nx g tools:sync-agents',
      });
    }
  }
  ...
  for (const file of existingAgentFiles) {
    ...
    if (!agents[agentName]) {
      errors.push({
        ...
        message:
          'Orphaned agent file (not in registry). Run: nx g tools:sync-agents',
      });
    }
  }

  // -----------------------------------------------------------------------
  // 2. Validate skill source files have matching generated output
  // -----------------------------------------------------------------------
  ...
  if (tree.exists(SKILLS_SOURCE_DIR)) {
    ...
    for (const file of sourceFiles) {
      ...
      for (const outputDir of [CLAUDE_SKILLS_DIR, CODEX_SKILLS_DIR]) {
        const outputPath = `${outputDir}/${name}/SKILL.md`;
        ...
        if (actual !== expected) {
          errors.push({
            file: outputPath,
            message: `Content out of sync with source .agents/skills/${file}. Run: nx g tools:sync-skills`,
          });
        }
      }
    }
  ...
  // -----------------------------------------------------------------------
  // 3. Validate settings.json (permissions + env + hooks)
  // -----------------------------------------------------------------------
  if (tree.exists(SETTINGS_JSON)) {
    try {
      ...
      if (settingsContent) {
        ...
        if (
          settingsRegistry.env &&
          Object.keys(settingsRegistry.env).length > 0
        ) {
        ...
        for (const [_name, hook] of Object.entries(hooksRegistry.hooks)) {
          ...
          if (!expectedHooks[trigger]) {
            expectedHooks[trigger] = [];
          }
        ...
        if (JSON.stringify(settings) !== JSON.stringify(expected)) {
          errors.push({
            file: SETTINGS_JSON,
            message:
              'Content does not match what registry would generate. Run: nx g tools:sync-settings',
          });
        }
      }
    } catch (e) {
      errors.push({
        file: SETTINGS_JSON,
        message: `Failed to validate settings.json: ${
