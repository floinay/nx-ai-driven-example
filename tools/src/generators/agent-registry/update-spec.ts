 ...
 *
 * Updates frontmatter fields in a spec, then regenerates INDEX.md.
 */
...
/**
 * Map legacy uppercase statuses to the new lowercase lifecycle values.
 */
...
export default async function updateSpec(
  ...
  if (!tree.exists(specPath)) {
    throw new Error(`Spec not found at ${specPath}.`);
  }

  let content = tree.read(specPath, 'utf-8');
  if (!content) {
    throw new Error(`Spec at ${specPath} is empty.`);
  }
    ...
    if (!VALID_STATUSES.includes(status as (typeof VALID_STATUSES)[number])) {
      throw new Error(
        `Invalid status "${schema.status}". Must be one of: ${VALID_STATUSES.join(
          ', '
        )} (or legacy: DRAFT, APPROVED, IN_PROGRESS, COMPLETE)`
      );
    }
