...
name: qa-analyst
description: >
  Quality gate before user approval. Checks build, tests, coverage (< 80% → feedback), edge cases, and architectural spot-check. Emits "qa-analyst: GREEN", "qa-analyst: FEEDBACK → [agent]", or "qa-analyst: ESCALATE → lead".
model: opus
